...
class GameLeaderboardViewModel(
    ...
    private val _usersAnswersList = MutableStateFlow<Map<ForeignUser, List<UUID?>>>(emptyMap())
    val usersAnswersList = _usersAnswersList.asStateFlow()
    ...
    init {
        _usersAnswersList.value = _room.value?.userList?.associateWith { emptyList() } ?: emptyMap()
                ...
                when (event) {
                    ...
                    is WsEvent.OutGoing -> when (event.message) {
                        ...
                        is WebSocketMessages.OutgoingMessage.UserAnswered -> {
                            ...
                            _usersAnswersList.update { current ->
